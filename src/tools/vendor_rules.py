...
MURATA_SIZE_MAP = {
    ...
    # Add specific GCJ size codes if they differ from GRM/GCM, e.g., "GCJ_SIZE_CODE": {...}
}
...
def parse_murata_mpn(mpn: str) -> dict:
    ...
    # Extract numeric size code: GRM155... → '15'. Now includes GCJ.
    size_match = re.match(r"(GRM|GCM|GRT|GCJ)(\d{2})\d", mpn) # Added GCJ
    if size_match:
    ...
    # Generate package_code (e.g., P0802, E0804) if packaging and pin_pitch available
    if parsed["packaging"] and parsed["pin_pitch"]: 
        pitch = int(parsed["pin_pitch"])
        type_letter = "P" if "paper" in parsed["packaging"] else "E"
        parsed["package_code"] = f"{type_letter}08{str(pitch).zfill(2)}" 

    # Generate SHAPE name (e.g., C0402, R0603)
    if parsed["length"] is not None and parsed["width"] is not None:
        component_type_char = ""
        # Infer component type based on MPN prefix
        if mpn.startswith(("GRM", "GCM", "GRT", "GCJ")): # Added GCJ
        ...
        if component_type_char:
            ...
            if eia_code:
                ...
                # Fallback heuristics for common sizes if exact float doesn't match
                if (rounded_L, rounded_W) == (0.6, 0.3): 
                    parsed["shape_name"] = f"{component_type_char}0201"
                elif (rounded_L, rounded_W) == (1.6, 0.8): 
...
#     size_match = re.match(r"(GRM|GCM|GRT|GCJ)(\d{2})\d", mpn) # Added GRT prefix
#     if size_match:
...
#         if mpn.startswith(("GRM", "GCM", "GRT", "GCJ")): # Murata Capacitors
#             component_type_char = "C" # Capacitor
